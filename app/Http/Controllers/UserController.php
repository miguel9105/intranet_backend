...
class UserController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $request->validate([
            'name_user'      => 'sometimes|string|max:255',
            'last_name_user' => 'sometimes|string|max:255',
            'birthdate'      => 'nullable|date',
            'email'          => ['sometimes', 'email', 'max:255', Rule::unique('users')->ignore($user->id)],
            'number_document'=> 'sometimes|string|max:255',
            'role_name'      => 'sometimes|string|exists:roles,name', // Validar rol si se envía
        ]);

        $user->update($request->all());
        
        // --- ¡AÑADIDO! Sincronizar el rol si se envió ---
        if ($request->has('role_name')) {
            // syncRoles es más seguro para 'update':
            // quita todos los roles anteriores y añade solo el nuevo.
